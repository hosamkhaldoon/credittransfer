        ...
        stage('🔍 SonarQube Analysis') {
            steps {
                script {
                    ...
                    // Try to load SonarQube credentials and run analysis
                    try {
                        ...
                        echo "   1. SonarQube credential 'sonartokenV3' doesn't exist in Jenkins"
                        echo "   2. SonarQube server is not running"
                        echo "   3. Network connectivity issues"
                        echo ""
                        echo "🔧 To fix SonarQube integration:"
                        ...
                        echo "   3. Make sure SonarQube container is running: docker ps | grep sonarqube"
                        echo ""
                ...
                always {
                    script {
                        ...
                        if (env.SONAR_HOST_URL) {
                            try {
                                // Try to use credentials for quality gate check
                                withCredentials([string(credentialsId: 'sonartokenV3', variable: 'SONAR_TOKEN')]) {
                                ...
                                echo "💡 This is expected if SonarQube credentials are not configured"
                            }
                        }
                    }
                }
